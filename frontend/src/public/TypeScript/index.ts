...
const elMinute = document.querySelector(".js-date-minutes") as HTMLSpanElement;
const elSecond = document.querySelector(".js-date-seconds") as HTMLSpanElement;
...
const handleErrorResponse = (elQuestionBox: genericsType<Element>, item:HTMLLIElement):void => {
    try{
        if(!item.getAttribute("class")?.includes("true")){
            userError += 1
            ...
            setTimeout(() => {
              ...
              item.classList?.remove("animation-error-response");
            }, 1000);
        }
    }catch(error){
        console.log(error)
...
function handleGameEndFn(type:boolean, dateType?: boolean):void{
    if(type){
...
async function handleResRequest (score: number, userError:number, dateType?:boolean, dateEndTye?: boolean ):Promise<void> {
...
function handleTrueResponse (elQuestionBox: genericsType<Element>, item:HTMLLIElement ):void {
    try{
        elQuestionBox?.classList.add("show-result")
...
const handleDate = (date: string | number):void => {
    minute = 1
